...
function ProductProvider({ children }) {
  ...
  const [loadingSkeletonItemOrders, setLoadingSkeletonItemOrders] =
    React.useState(true);
  ...
  const { productCard, setProductCard } = CallForProducts(
    setLoadingSkeletonCard,
  );
  ...
  const { isUserSingIn, updateIsUserSingIn: setIsUserSingIn } =
    LocalStorageLogIn();

  // Info User On : This states saves the information of the user that its on in his account
  const { currentUser, updateCurrenUser: setCurrentUser } =
    LocalStorageConcurrenUser();
  ...
  const [isAccountSideOpenSingUp, setIsAccountSideOpenSingUp] =
  ...
  const filterProductsByCategory = productCard.filter((product) => {
    if (categorySelected) {
  ...
  const filterProducts = filterProductsByCategory.filter((product) => {
